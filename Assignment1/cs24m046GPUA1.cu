...
__global__ void CalculateInvertedGrayScale(long int *d_red, long int *d_green, long int *d_blue, long int *T1, int m, int n){
  int id = blockDim.x * blockIdx.x + threadIdx.x, irow, col;
  long int d_grey;
  // printf("%d ", id);
  if(id < m*n){                         // to overcome outoff-bound excess
    d_grey = (d_red[id] + d_green[id] + d_blue[id])/3;
    irow = m - (id/n) - 1;
    col = id%n;
    // printf("%d %d %lld \n", irow, col, d_grey);
    T1[irow*n + col] = d_grey;
  }
}

__global__ void CalculateThomasTransformation(long int *d_red, long int *d_green, long int *d_blue, long int *T2, int m, int n){
  int id = blockDim.x * blockIdx.x + threadIdx.x;
  long int res;
  if(id < m*n){                         // to overcome outoff-bound excess
    res = (long int)(0.5 * d_red[id]) + (long int)sqrt((float)d_green[id]) + d_blue[id];
    T2[id] = res;
    // printf("%lld\n", res);
  }
}
...
{
    const char *inputFileName = argv[1]; //"/content/1";
